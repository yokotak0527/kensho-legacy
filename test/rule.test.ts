...
describe('default rules', () => {
  ...
  test('equal', () => {
    expect(Kensho.validate('equal', 'hoge', { others:'hoge', isInput : false })).toBeTruthy()
    expect(Kensho.validate('equal', 'hoge', { others:['hoge', 'hoge'], isInput : false })).toBeTruthy()
    expect(Kensho.validate('equal', 'hoge', { others:'fuga', isInput : false })).toBeFalsy()
    expect(Kensho.validate('equal', 'hoge', { others:['hoge', 'fuga'], isInput : false })).toBeFalsy()
  })
  test('letters', () => {
    expect(Kensho.validate('letters', 'text', { range : { min : 3 } })).toBeTruthy()
    expect(Kensho.validate('letters', 'text', { range : { min : 4 } })).toBeTruthy()
    expect(Kensho.validate('letters', 'text', { range : { min : 5 } })).toBeFalsy()
    expect(Kensho.validate('letters', 'text', { range : { max : 5 } })).toBeTruthy()
    expect(Kensho.validate('letters', 'text', { range : { max : 4 } })).toBeTruthy()
    expect(Kensho.validate('letters', 'text', { range : { max : 3 } })).toBeFalsy()
    expect(Kensho.validate('letters', 'text', { range : { min : 4, max : 5 } })).toBeTruthy()
    expect(() => { Kensho.validate('letters', 'text', { range : { min : 10, max : 5 } }) }).toThrow()
    expect(() => { Kensho.validate('letters', 'text', { range : {} }) }).toThrow()
  })
})
